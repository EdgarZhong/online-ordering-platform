...
| 任务编号 | 关联用户故事 | 任务描述 | 负责人 | 状态 | 预估工作量评分 (1-3) | 备注 |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 301 | USa_C1 | **API 设计**: 撰写消费者端核心流程的 API 文档 (`API.md`) | A | 待办 | 2 | **API-First, Doc-First** |
| 302 | USa_C2 | **API 实现**: `GET /api/restaurants` - 获取所有餐厅列表 | A | 待办 | 2 | M1: 只读 API 雏形 |
| 303 | USa_C2 | **API 实现**: `GET /api/restaurants/{id}` - 获取单个餐厅详情 | A | 待办 | 1 | M1: 只读 API 雏形 |
| 304 | USa_C3 | **API 实现**: `GET /api/restaurants/{id}/menus` - 获取餐厅的菜单列表 | B | 待办 | 2 | M1: 只读 API 雏形 |
| 305 | USa_C3 | **API 实现**: `GET /api/menus/{menuId}/items` - 获取菜单下的菜品项 | B | 待办 | 2 | M1: 只读 API 雏形 |
| 306 | USa_C4 | **API 实现**: `POST /api/orders` - 创建新订单 (含事务) | C | 待办 | 3 | M3: 订单写入 |
| 307 | USa_C5 | **API 实现**: `GET /api/orders/{orderId}` - 查询订单状态 | C | 待办 | 2 | M3: 订单状态查询 |
| 308 | USa_C1 | **前端**: 搭建 Vue 项目骨架，配置路由 (`vue-router`) | D | 待办 | 2 | M2: 前端路由打通 |
| 309 | USa_C2 | **前端**: 开发 `RestaurantList` 组件并与 API 联调 | D | 待办 | 3 | M2: 模拟下单 |
| 310 | USa_C3 | **前端**: 开发 `RestaurantDetail` 和 `MenuItemCard` 组件 | D | 待办 | 3 | M2: 模拟下单 |
| 311 | USa_C4 | **前端**: 开发购物车 (`CartDrawer`) 及状态管理 (Pinia/Vuex) | D | 待办 | 3 | M2: 模拟下单 |
| 312 | USa_C4 | **前端**: 开发 `CheckoutForm` 组件并与下单 API 联调 | D | 待办 | 2 | M3: 订单写入 |
| 313 | NFR | **事务**: 实现 `createOrder` 的 JDBC 事务，确保原子性 | A/C | 待办 | 3 | 核心业务逻辑 |
...
| 任务编号 | 关联用户故事 | 任务描述 | 负责人 | 状态 | 预估工作量评分 (1-3) | 完成日期 | 备注 |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 401 | US-Admin-Orders | **订单列表页**：分页/筛选/排序（下单时间倒序）；JSP 视图与 Servlet 列表 GET | B/E | 已完成 | 2 | 2025-11-16 | `admin/orders.jsp` + `OrdersAdminServlet` |
| 402 | US-Admin-Orders | **订单详情页**：菜单-菜品分组展示；状态流转按钮与校验 | B/E | 已完成 | 2 | 2025-11-16 | `admin/order-detail.jsp` + 详情 GET |
| 403 | US-Admin-Orders | **DAO**：`OrderAdminDAO/Impl` 分页查询、总数统计、详情、状态更新（严格隔离） | A/B | 已完成 | 3 | 2025-11-16 | SQL 变量以 `sql_` 前缀命名 |
| 404 | US-Admin-Orders | **状态流转**：PENDING→PROCESSING→COMPLETED；任意阶段→CANCELLED；完成/取消不可变更 | A/B | 已完成 | 2 | 2025-11-16 | 现按 `schema.sql` 状态枚举 |
| 405 | US-Kitchen-Board | **厨房面板**：`/admin/kitchen` JSP 分栏（待处理/备餐/待取）+ 快捷操作 | B/E | 待办 | 3 |  | 轮询/手动刷新均可 |
